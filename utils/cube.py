#!/usr/bin/env python2
# https://pastebin.com/raw/HGzF8n9k
#####################################################################
...
from draw import _newPlot, _canvasToImage
...
def crazy_rotation(ind,Nind):
    return quaternion_to_matrix(0.5*np.sin(np.pi*2*ind*Nind**-1.*np.array([1,2,3])))
...
## The cube vertices
vs = np.reshape(np.mgrid[-1:2:2,-1:2:2,-1:2:2].T, (8,3))
...
def draw(ind=0, size=SCREEN_SIZE, Nind=90):
    global vvs
    ...
    ## Plot the edges.
    for j,k in ed:
        ax.plot(pt[[j,k],0], pt[[j,k],1], 'g-', lw=2, color='w')

    ## Plot the vertices.
    # ax.plot(pt[:,0], pt[:,1], 'bo', color='w')
